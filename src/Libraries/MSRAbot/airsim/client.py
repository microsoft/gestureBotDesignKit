...
# -----------------------------------  Robot APIs ---------------------------------------------
class RobotClient(VehicleClient, object):
    def __init__(self, ip = "", port = 41451, timeout_value = 3600):
        super(RobotClient, self).__init__(ip, port, timeout_value)

    def setRobotControls(self, controls, vehicle_name = ''):
        """
        Control the Robot using throttle, steering, brake, etc.

        Args:
            controls (RobotControls): Struct containing control values
            vehicle_name (str, optional): Name of vehicle to be controlled
        """
        self.client.call('setRobotControls', controls, vehicle_name)

    def setRobotPose(self, pose, vehicle_name = ''):
        """
        Set the Robot pose.

        Args:
            pose (RobotPose): Struct containing joint targets
            vehicle_name (str, optional): Name of vehicle to be controlled
        """
        self.client.call('setRobotPose', pose, vehicle_name)

    def getRobotState(self, vehicle_name = ''):
        """
        Args:
            vehicle_name (str, optional): Name of vehicle
        ...
        Returns:
            RobotState:
        """
        state_raw = self.client.call('getRobotState', vehicle_name)
        return RobotState.from_msgpack(state_raw)

    def getRobotControls(self, vehicle_name=''):
        """
        Args:
            vehicle_name (str, optional): Name of vehicle
        ...
        Returns:
            RobotControls:
        """
        controls_raw = self.client.call('getRobotControls', vehicle_name)
        return RobotControls.from_msgpack(controls_raw)

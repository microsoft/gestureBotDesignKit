...
from .servo import MSRAbotServo
from .hardware import MSRAbotHardware
from .airsimclient import AirSim

# -----------------------------------------------------------------------------
#
class MSRAbot(webSocket.HttpServerWrapper):
    # -----------------------------------------------------------------------------
    #
    def __init__(self, controller, **args):
        self.context = args.get('context')
        if (self.context is None):
            self.context = "MSRAbot"
        ...
        self.staticElbow = True
        self.staticElbowAngleLeft = -(math.pi) - 0.25
        self.staticElbowAngleRight = +0.25
        ...
        self.servoMap = [ 'torso', 'neck', 'head', 'lshoulder', 'larm', 'lhand', 'rshoulder', 'rarm', 'rhand' ]
        self.servoTargets = {}
        ...
        #
        # set a sampling rate for our MSRAbot. Because servo angles are not linear to labanotation 
        ...
        #
        # physical MSRAbot robot
        self.hardware = MSRAbotHardware(self)
        ...
        #
        # start a thread for servo updates
        self.fSimulateServos = False
        if (self.fSimulateServos):
            self.servos = {}

            for key in self.servoMap:
                self.servos[key] = MSRAbotServo(key)

            self.fKeepUpdatingServos = True
            self.thread = threading.Thread(target = self.threadSimulateServoPositions, args=(1,))
            self.thread.start()

        #
        # if requested, create and initialize http server to access MSRAbot
    ...
    def handleSetManualServo(self, data):
        try:
            ...
            self.setManualServo(key, target, duration)
        except Exception as e:
    ...
    def setManualServo(self, key, target, duration):
        if (self.hardware is not None):
            self.hardware.setManualServo(key, target, duration)
    ...
    def calculateServoTargets(self, keyFrame):
        ...
        if (self.staticElbow):
            theta = self.staticElbowAngleLeft
        ...
        if (self.staticElbow):
            theta = self.staticElbowAngleRight
    ...
    def setServos(self, servos, duration):
        for key in servos:
            ...
            if ((key in self.servoTargets) and (round(self.servoTargets[key]['target'], 3) != target)):
                ...
                if (self.fSimulateServos):
                    self.servos[key].setTarget(target, duration)
    
                if ((self.hardware is not None) and (self.hardware.connected())):
                    if (key in self.servoMap):
                        self.hardware.setManualServo(key, target, duration)
    ...
    # -----------------------------------------------------------------------------
    #
    def threadSimulateServoPositions(self, name):
        frames = 0
        timeStartGesture = timeFPS = utilities.getTime()
        targetFPS = 30
        invFPS = (1.0 / targetFPS)

        while self.fKeepUpdatingServos:
            timeStartFrame = utilities.getTime()

            #
            # update all servos
            for key in self.servos:
                servo = self.servos[key]

                servo.update(timeStartFrame)
            ...
            diff = invFPS - (utilities.getTime() - timeStartFrame)
            if (diff > 0.0):
                time.sleep(diff)

...
var UIControls = function (app)
{
    ...
    this.servoKeys = [ 'head', 'neck', 'torso', 'hip', 'rshoulder', 'rarm', 'relbow', 'rhand', 'lshoulder', 'larm', 'lelbow', 'lhand' ];
    this.servos = [ "Head", "Neck", "Torso", 'Hip', "Right Shoulder", "Right Upper Arm", "Right Elbow", "Right Hand", "Left Shoulder", "Left Upper Arm", "Left Elbow", "Left Hand" ];
    this.servoSpeed = 0.4;
}
//--------------------------------------------------------------------------------------------
...
UIControls.prototype.setupHardwareOptions = function ()
{
    ...
    var objRebootServos = document.getElementById('btnRebootServos');
    if (objRebootServos) {
        objRebootServos.onclick = function () {
            objConnect.disabled = true;

            var msg = JSON.stringify({ msgType: "hardware", "hardware": { "command": "rebootServos" } });
...
{
    this.setElementDisabledState("lblServoAdjustment", enabled, true);
    this.setElementDisabledState("sliderServoAdjustment", enabled, true);
    this.setElementDisabledState("lblServoAdjustmentValue", enabled, true);
}
//--------------------------------------------------------------------------------------------
...
UIControls.prototype.setupMiscellaneousOptions = function ()
{
    ...
    obj = document.getElementById('renderShadow');
    if (obj) {
        obj.checked = true;
        obj.onclick = function (event) {
            ...
            if (scope.app != null)
                scope.app.renderShadow(fChecked);
        };
    }

    obj = document.getElementById('useHighResModel');
    if (obj) {
        obj.onclick = function (event) {
            var fChecked = event.target.checked ? true : false;

            if (scope.app != null)
                scope.app.useHighResModel(fChecked);
        };
    }

    obj = document.getElementById('hideShell');
    if (obj) {
        obj.onclick = function (event) {
            var fChecked = event.target.checked ? true : false;

            if (scope.app != null)
                scope.app.hideShell(fChecked);
        };
    }
    ...
    if (obj3) {
        obj3.oninput = function (event) {
            ...
            if (scope.app != null)
                scope.app.changeGestureBotOpacity(s);
        }
    }
}
//--------------------------------------------------------------------------------------------
...
{
    this.setElementDisabledState("lblLWheel", enabled, true);
    this.setElementDisabledState("sliderLWheel", enabled);
    this.setElementDisabledState("lblLWheelValue", enabled, true);

    this.setElementDisabledState("lblRWheel", enabled, true);
    this.setElementDisabledState("sliderRWheel", enabled);
    this.setElementDisabledState("lblRWheelValue", enabled, true);

    this.setElementDisabledState("chkSyncWheels", enabled);
    this.setElementDisabledState("lblSyncWheels", enabled, true);
    this.setElementDisabledState("btnWheelStop", enabled);
...
UIControls.prototype.setupServoOverwrite = function ()
{
    ...
    obj = document.getElementById('chkServoOverwrite');
    if (obj) {
        obj.checked = false;
    ...
    for (var i = 0; i < this.servos.length; i++) {
        ...
        input.className = "coloredRange disabled"; // disabled by default
        ...
        input.style.setProperty("background", "#e0e0e0");
        input.disabled = true;
...
UIControls.prototype.setGestureInfo = function (gestureInfo)
{
    ...
    //
    // the gestureBot will draw a more detailed frame time line...
...
//--------------------------------------------------------------------------------------------
UIControls.prototype.setServoAngleLimits = function (servoLimits)
{
    //
...
UIControls.prototype.updateHardware = function (data)
{
    ...
    if (status) {
        ...
        obj = document.getElementById('btnRebootServos');
        if (obj) {
            obj.disabled = connected ? false : true;
        }
...
//--------------------------------------------------------------------------------------------
UIControls.prototype.setPlayStatus = function (isPlaying)
{
...
UIControls.prototype.updateCanvasDirPad = function (mousePos)
{
    ...
    if (this.canvasMouseDown && (mousePos != null) && this.dirPadEnabled) {
        ...
        if (this.fSynchronizeWheels) {
            ...
            //ctx.fillText("" + s.toFixed(2) + ", " + a.toFixed(2), centerX, centerY);
        }
...
//--------------------------------------------------------------------------------------------
UIControls.prototype.getServoBackgroundCSS = function (key)
{
    var green = "#b2ffb2";
    var red = "#ffb2b2";

    if (key in this.servoLimits) {
        var min = parseFloat(180 + this.servoLimits[key].min);
        var max = parseFloat(180 + this.servoLimits[key].max);

        var min_p = parseInt((min / 360.0) * 100.0);
        var max_p = parseInt((max / 360.0) * 100.0);

        var css = "linear-gradient(90deg, " + red + " " + min_p + "%, " + green + " 0, " + green + " " + max_p + "%, " + red + " 0)";

        return css;
    }

    return "linear-gradient(90deg, " + red + " 20%, " + green + " 0, " + green + " 80%, " + red + " 0)";
}
//--------------------------------------------------------------------------------------------
UIControls.prototype.setServoOverwriteState = function ()
{
    ...
    for (var i = 0; i < this.servoKeys.length; i++) {
        ...
        if (this.servoOverwrite) {
            this.servoSliders[key].className = "coloredRange enabled";
            this.servoSliders[key].style.setProperty("background", this.getServoBackgroundCSS(key));
        } else {
            this.servoSliders[key].className = "coloredRange disabled";
            this.servoSliders[key].style.setProperty("background", "#e0e0e0");
        }
...
//--------------------------------------------------------------------------------------------
UIControls.prototype.mapPositionToXL320 = function (target)
{
    //
    ...
    return parseInt(target / 300 * 1023);
}
//--------------------------------------------------------------------------------------------
UIControls.prototype.calculateDuration = function (position, target, velocity)
{
    var goal = this.mapPositionToXL320(target);
    var position = this.mapPositionToXL320(position);

    var distance = Math.abs(goal - position);
    var velocity = (0x3ff * this.servoSpeed);
    var coeff = 2.0;
    var speed = velocity * coeff;
...
UIControls.prototype.onServoSliderUpdate = function (value, idx, lbl)
{
    ...
    if (_app && _app.gestureBot) {
        var key = this.servoKeys[idx];
        var servo = _app.gestureBot.servos[key];

        var currentRotation = servo.getRotation();
        var velocity = (0x3ff * this.servoSpeed);
        var duration = this.calculateDuration(currentRotation, target, velocity);

        if (key in this.servoLimits) {
            var min = parseFloat(this.servoLimits[key].min);
            var max = parseFloat(this.servoLimits[key].max);
        ...
        document.getElementById(lbl).textContent = "" + target + "\xB0";
        event.target.value = target;

        servo.setTarget(target, duration);

        target = THREE.MathUtils.degToRad(target);
        if (target < -180.0)
            target += 360.0;
...
//--------------------------------------------------------------------------------------------
UIControls.prototype.setElementDisabledState = function (id, enable, changeColor = false)
{
    ...
    if (changeColor)
        obj.style.color = enable ? this.clrControlEnabled : this.clrControlDisabled;
}
//--------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------
UIControls.prototype.allReady = function ()
{
    if (false) {
        var obj;
